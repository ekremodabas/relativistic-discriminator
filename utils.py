...
def sample_fid(generator, it, args, batch_size=500):
    ...
    with torch.no_grad():
        ...
        np.savez_compressed(f"samples/{args.dataset}_{args.loss_type}_n_d_{args.d_iter}_betas_{args.beta1}_{args.beta2}_b_size_{args.batch_size}_lr_{args.lr}_{it+1}" +  ( "_noBN" if args.no_BN else "") + ("_alltanh" if args.all_tanh else ""), images=arr)
